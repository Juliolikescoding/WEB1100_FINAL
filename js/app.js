...
const  doggos = document.querySelectorAll('select')
const  pupper_imgs = document.querySelectorAll('img')

function random(){
    const api_url = `${dog_api}breeds/list/all`
    $.ajax({
      url: api_url,
      dataType: "json",
      success: data => {
...
function pupper(){
    const img_url = `${dog_api}breed/hound/images`;
    $.ajax({
        url: img_url,
        dataType: "json",
        success: data =>{
...
viewDog.addEventListener('click', random(pupper()))
